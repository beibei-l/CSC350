...
// vertex shader.  The only difference between this and ripple is that the
// color of the mesh is passed as a uniform to the fragment shader.
//
...
void display(void)
{
  ...
  GLint ctmLocation = glGetUniformLocation( shaderProgram, "ctm" );
  glUniformMatrix4fv( ctmLocation, 1, GL_FALSE, glm::value_ptr(ctm) ); 
  GLint timeLocation = glGetUniformLocation( shaderProgram, "time" );
  glUniform1f( timeLocation, rippleTime ); 

  glm::vec3 fColor = glm::vec3( 1.0, 1.0, 0.0 );
  GLint fColorLocation = glGetUniformLocation( shaderProgram, "fColor" );
  glUniform3fv( fColorLocation, 1, glm::value_ptr( fColor ) );

  // draw the color cube as lines
